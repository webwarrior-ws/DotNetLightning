...
let genericTLVGen(known: list<uint64>) =
    gen {
        let! t =
            Arb.generate<uint64>
            |> Gen.filter(fun v ->
                not <| (List.exists (fun knownInt -> v = knownInt) known)
            )
...
let openChannelGen =
    let constructor =
        fun arg1 arg2 arg3 arg4 arg5 arg6 arg7 arg8 arg9 arg10 arg11 arg12 arg13 arg14 arg15 arg16 arg17 arg18 arg19 ->
            {
    ...
    <*> Gen.oneof
            [
                ...
                gen {
                    ...
                    return
                        [|
                            OpenChannelTLV.UpfrontShutdownScript shutdownScript
                        |]
                }
            ]
...
let acceptChannelGen =
    ...
    constructor <!> temporaryChannelGen
    <*> moneyGen
    <*> lnMoneyGen
    <*> moneyGen
    <*> lnMoneyGen
    ...
    <*> Gen.oneof
            [
                gen {
                    ...
                    return
                        [|
                            AcceptChannelTLV.Unknown genericTLV
                        |]
                }
                gen {
                    let! shutdownScript = (Gen.optionOf shutdownScriptPubKeyGen)
...
let updateFulfillHTLCGen =
    gen {
        let! c = ChannelId <!> uint256Gen
        let! htlc = HTLCId <!> Arb.generate<uint64>

        let! paymentPreimage =
            PaymentPreimage.Create <!> bytesOfNGen PaymentPreimage.LENGTH
...
let updateFailMalformedHTLCGen =
    gen {
        let! c = ChannelId <!> uint256Gen
        let! htlc = HTLCId <!> Arb.generate<uint64>
...
let private dataLossProtectGen =
    gen {
        let! perCommitmentSecret = perCommitmentSecretGen
        let! pk = perCommitmentPointGen
...
let private ipV4AddressGen =
    gen {
        ...
        return
            NetAddress.IPv4
                {
                    IPv4Or6Data.Addr = addr
                    Port = port
                }
    }

let private ipV6AddressGen =
    gen {
...
let private onionV2AddressGen =
    gen {
        ...
        return
            NetAddress.OnionV2
                {
                    OnionV2EndPoint.Addr = addr
                    Port = port
                }
    }

let private onionV3AddressGen =
    gen {
...
let private unsignedChannelAnnouncementGen =
    gen {
        let! g = featuresGen
        let! ch = uint256Gen
        let! s = ShortChannelId.FromUInt64 <!> Arb.generate<uint64>
...
let private unsignedChannelUpdateGen =
    gen {
        let! ch = uint256Gen
        let! s = ShortChannelId.FromUInt64 <!> Arb.generate<uint64>
...
let queryShortChannelIdsGen =
    gen {
        ...
        let! unknownTLVs =
            (genericTLVGen([ 1UL ]) |> Gen.map(QueryShortChannelIdsTLV.Unknown))
            |> Gen.optionOf
            |> Gen.map(Option.toArray)

        let tlvs = [ knownTLVs; unknownTLVs ] |> Array.concat

        return
            {
                QueryShortChannelIdsMsg.ChainHash = s
...
let replyShortChannelIdsEndGen =
    gen {
...
let private queryChannelRangeTLVGen =
    Gen.frequency
        [|
            (1,
             Arb.generate<uint8>
             |> Gen.filter(fun x -> 0xfduy > x)
             |> Gen.map(QueryOption.Create >> QueryChannelRangeTLV.Opt))
            (1, genericTLVGen([ 1UL ]) |> Gen.map(QueryChannelRangeTLV.Unknown))
        |]

let queryChannelRangeGen: Gen<QueryChannelRangeMsg> =
    gen {
        let! chainHash = chainHashGen
...
let private checksumPairsGen n =
    gen {
        let! pairs =
            (Arb.generate<uint32>, Arb.generate<uint32>)
            ||> Gen.map2(fun a b ->
...
let private replyChannelRangeTLVGen n =
    Gen.frequency
        [
            ...
            (1,
             genericTLVGen([ 1UL; 3UL ])
             |> Gen.map(ReplyChannelRangeTLV.Unknown))
        ]

let replyChannelRangeGen: Gen<ReplyChannelRangeMsg> =
    gen {
        let! ch = chainHashGen
        let! firstBlockNum = Arb.generate<uint32> |> Gen.map(BlockHeight)
        let! nBlocks = Arb.generate<uint32>
...
let gossipTimestampFilterGen: Gen<GossipTimestampFilterMsg> =
    gen {
        let! ch = chainHashGen
...
let private onionPayloadTlvGen =
    Gen.frequency
        [
            (1,
             genericTLVGen([ 2UL; 4UL; 6UL; 8UL ])
             |> Gen.map(HopPayloadTLV.Unknown))
            (4, hopPayloadTlvGen)
        ]
...
let tlvOnionPayloadGen =
    gen {
        let! tlv =
            Gen.nonEmptyListOf onionPayloadTlvGen |> Gen.map(List.toArray)
