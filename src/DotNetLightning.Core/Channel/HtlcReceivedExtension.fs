    ...
    static member TryExtractParameters
        ...
        let parseResult =
            ...
            <| seqParser {
                ...
                let! _shouldbecltv = SeqParser.next()
                do! checkOpCode OpcodeType.OP_CHECKLOCKTIMEVERIFY
                do! checkOpCode OpcodeType.OP_DROP
                do! checkOpCode OpcodeType.OP_CHECKSIG
                do! checkOpCode OpcodeType.OP_ENDIF
                // we can't verify furthur because it changes based on anchor/non-anchor
                return
                    {
...
type internal HtlcReceivedExtension(paymentPreimageOpt: option<PaymentPreimage>) =
    inherit BuilderExtension()

    override __.Match(coin: ICoin, _input: PSBTInput) : bool =
        (HtlcReceivedParameters.TryExtractParameters(coin.GetScriptCode()))
            .IsSome
    ...
    override __.Sign
        ...
        match
            HtlcReceivedParameters.TryExtractParameters scriptPubKey,
            paymentPreimageOpt
            with
        ...
        | Some parameters, Some _preimage ->
            ...
            | :? TransactionSignature as remoteSignature when
                ...
                ->
                match signer.Sign(parameters.LocalHtlcPubKey.RawPubKey()) with
    ...
    override __.EstimateScriptSigSize(_coin: ICoin) : int =
        ...
        | Some _preimage ->
            (ChannelConstants.MaxSignatureSize * 2) + 1 + PaymentPreimage.LENGTH
        | None -> ChannelConstants.MaxSignatureSize + 1
    ...
    override __.Finalize(inputSigningContext: InputSigningContext) =
        ...
        let parameters =
            match (HtlcReceivedParameters.TryExtractParameters scriptPubKey)
                with
        ...
        | Some preImage ->
            ...
            match remoteSignatureOpt, localSignatureOpt with
            | Some remoteKeySignature, Some localKeySignature ->
                txIn.FinalScriptSig <-
                    Script
                        [
        ...
        | None ->
            if txIn.PartialSigs.Count <> 0 then
                ...
                match revocationSignatureOpt with
                | Some revocationKeySignature ->
                    txIn.FinalScriptSig <-
                        Script
                            [
                                Op.GetPushOp(
                    ...
                    | Some remoteKeySignature ->
                        txIn.FinalScriptSig <-
                            Script
                                [
                                    Op.GetPushOp(
                                        remoteKeySignature.Value.ToBytes()
                                    )
                                    Op.op_Implicit OpcodeType.OP_0
                                ]
                    | _ -> ()
