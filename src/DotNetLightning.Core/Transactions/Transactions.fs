...
type CommitTx =
    ...
    interface ILightningTx with
        member this.Value: PSBT = this.Value
...
module private HTLCHelper =
    let createHTLCWitScript
        (
            localSig: TransactionSignature,
            remoteSig: TransactionSignature,
            witScript: Script,
    ...
    let finalize
        ...
        let finalWit =
            createHTLCWitScript(localSig, remoteSig, witScript, paymentPreimage)
        ...
        let txIn =
            psbt
                ...
                .Inputs.FindIndexedInput(coin.Outpoint)
        ...
        | true ->
            ...
            htlcTx.Value.Inputs.[htlcTx.WhichInput]
                .PartialSigs.Clear()

            htlcTx.Value.ExtractTransaction() |> Ok
        | false ->
...
type HTLCSuccessTx =
    ...
    member this.GetRedeem() =
        this.Value.Inputs.[HTLCSuccessTx.WhichInput]
            .WitnessScript

    member this.Finalize(localPubkey, remotePubKey, paymentPreimage) =
        let localSig =
            this.Value.Inputs.[HTLCSuccessTx.WhichInput]
                .PartialSigs.[localPubkey]

        let remoteSig =
            this.Value.Inputs.[HTLCSuccessTx.WhichInput]
                .PartialSigs.[remotePubKey]
...
type ClaimHTLCSuccessTx =
    | ClaimHTLCSuccessTx of PSBT
...
type ClaimHTLCTimeoutTx =
    | ClaimHTLCTimeoutTx of PSBT
...
type ClaimP2WPKHOutputTx =
    | ClaimP2WPKHOutputTx of PSBT
...
type ClaimDelayedOutputTx =
    | ClaimDelayedOutputTx of PSBT
...
type ClosingTx =
    ...
    interface ILightningTx with
        member this.Value = this.Value
        member this.WhichInput: int = this.WhichInput
...
type SortableTxOut =
    {
        TxOut: TxOut
        UpdateAddHTLC: option<UpdateAddHTLCMsg>
    }
    ...
    override this.GetHashCode() =
        ...
        | Some htlc ->
            Array.append (this.TxOut.ToBytes()) (htlc.ToBytes()) |> hash
    ...
    interface IComparable with
        member this.CompareTo(obj: obj) : int =
            match obj with
            | :? SortableTxOut as other ->
                (this :> IComparable<SortableTxOut>)
                    .CompareTo(other)
            | _ -> 1
    ...
    interface IComparable<SortableTxOut> with
        member this.CompareTo(obj: SortableTxOut) =
            ...
            if (c1 <> 0) then
                ...
                let c2 =
                    BytesComparer.Instance.Compare(
                        txout1.ScriptPubKey.ToBytes(),
                        txout2.ScriptPubKey.ToBytes()
                    )
                ...
                if (c2 <> 0) then
                    ...
                    | Some a, Some b ->
                        let c3 =
                            a.CLTVExpiry.Value.CompareTo(b.CLTVExpiry.Value)

                        if c3 <> 0 then
                            c3
                        else
                            let c4 =
                                a.PaymentHash.Value.CompareTo(
                                    b.PaymentHash.Value
                                )
...
module Transactions =
    ...
    let private trimOfferedHTLCs
        ...
        |> List.map snd
        |> List.filter(fun v ->
            (v.Amount.ToMoney()) >= (dustLimit + htlcTimeoutFee)
        )
    ...
    let sortTxOut(txOutsWithMeta: list<(TxOut * Option<UpdateAddHTLCMsg>)>) =
        txOutsWithMeta
        |> List.sortBy(fun txom ->
            {
                SortableTxOut.TxOut = (fst txom)
                UpdateAddHTLC = (snd txom)
            }
        )
        |> List.map(fst)
    ...
    let makeCommitTx
        ...
        let (toLocalAmount, toRemoteAmount) =
            if localIsFunder then
                (spec.ToLocal.Satoshi |> Money.Satoshis) - commitFee,
                spec.ToRemote.Satoshi |> Money.Satoshis
            else
        ...
        let tx =
            ...
            let txb, outAmount =
                let txb =
                    (createDeterministicTransactionBuilder network)
                        ...
                        .AddCoin(
                            inputInfo,
                            CoinOptions(Sequence = Nullable sequence)
                        )
            ...
            txb.SendFees(actualFee).BuildTransaction true
    ...
    let private findScriptPubKeyIndex (tx: Transaction) (spk: Script) =
        tx.Outputs
        |> List.ofSeq
        |> List.findIndex(fun o -> o.ScriptPubKey = spk)

    let checkTxFinalized
        (psbt: PSBT)
        (inputIndex: int)
        (additionalKnownSigs: seq<(PubKey * TransactionSignature)>)
        : Result<FinalizedTx, _> =
        let checkTxFinalizedCore(psbt: PSBT) : Result<_, _> =
            match psbt.TryFinalize() with
            | false, e ->
                (sprintf
                    "failed to finalize psbt Errors: %A \n PSBTInput: %A \n base64 PSBT: %s"
                    e
                    psbt.Inputs.[inputIndex]
                    (psbt.ToBase64()))
                |> FailedToFinalizeScript
                |> Error
            | true, _ -> psbt.ExtractTransaction() |> FinalizedTx |> Ok

        match psbt.Inputs.[inputIndex].GetTxOut() with
        | null ->
            failwithf
                "Bug: prevout does not exist in %d for psbt: %A"
                inputIndex
                (psbt)
        | _ ->
            additionalKnownSigs
    ...
    let checkSigAndAdd
        ...
        if (tx.Value.IsAllFinalized()) then
            ...
            let checker =
                TransactionChecker(globalTx, tx.WhichInput, spentOutput)

            let sigHash =
                checker.Transaction.GetSignatureHash(
                    scriptCode,
                    checker.Index,
                    signature.SigHash,
                    checker.SpentOutput,
                    HashVersion.WitnessV0,
                    checker.PrecomputedTransactionData
                )

            if pk.Verify(sigHash, signature.Signature) then
                tx.Value.Inputs.[tx.WhichInput]
                    .PartialSigs.AddOrReplace(pk, signature)
    ...
    let signCore(tx: ILightningTx, key: Key, enforceLowR, sigHash) =
        ...
        | Error(InvalidSignature signature) ->
            failwithf
                "Failed to check signature. (%A) This should never happen."
                signature
    ...
    let makeClosingTx
        ...
        if (not spec.OutgoingHTLCs.IsEmpty) || (not spec.IncomingHTLCs.IsEmpty) then
            HTLCNotClean(
                (spec.OutgoingHTLCs |> Map.toList |> List.map(fst))
                @ (spec.IncomingHTLCs |> Map.toList |> List.map(fst))
            )
            |> Error
        else
            let toLocalAmount, toRemoteAmount =
                ...
                seq {
                    ...
                    if toRemoteAmount >= dustLimit then
                        yield
                            TxOut(
                                toRemoteAmount,
                                remoteDestination.ScriptPubKey()
                            )
                }
