...
module Data =
    type NegotiatingState =
        ...
        member this.HasEnteredShutdown() : bool =
            this.LocalRequestedShutdown.IsSome
            && this.RemoteRequestedShutdown.IsSome
...
type SavedChannelState =
    {
        ...
        RemoteCurrentPerCommitmentPoint: Option<PerCommitmentPoint>
    }
    ...
    member internal this.GetOutgoingHTLCCrossSigned
        (remoteNextCommitInfo: RemoteNextCommitInfo)
        (htlcId: HTLCId)
    ...
    member internal this.GetIncomingHTLCCrossSigned
        (remoteNextCommitInfo: RemoteNextCommitInfo)
        (htlcId: HTLCId)
        : Option<UpdateAddHTLCMsg> =
        let remoteSigned =
