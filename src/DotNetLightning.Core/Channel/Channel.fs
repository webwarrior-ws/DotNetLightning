...
type ChannelWaitingForFundingSigned =
    ...
    member this.ApplyFundingSigned
        (msg: FundingSignedMsg)
        : Result<FinalizedTx * Channel, ChannelError> =
        result {
            let! finalizedLocalCommitTx =
                let theirFundingPk =
                    this.StaticChannelConfig.RemoteChannelPubKeys.FundingPubKey.RawPubKey
                        ()

                let _, signedLocalCommitTx =
                    this.ChannelPrivKeys.SignWithFundingPrivKey
                        this.LocalCommitTx.Value

                let remoteSigPairOfLocalTx =
                    (theirFundingPk,
...
and ChannelWaitingForFundingCreated =
    ...
    member this.ApplyFundingCreated
        ...
        result {
            ...
            let theirSigPair =
                (this.RemoteChannelPubKeys.FundingPubKey.RawPubKey(),
                 remoteTxSig)
            ...
            let staticChannelConfig =
                {
                    ...
                    LocalStaticShutdownScriptPubKey =
                        this.LocalStaticShutdownScriptPubKey
                    RemoteStaticShutdownScriptPubKey =
                        this.RemoteStaticShutdownScriptPubKey
...
and ChannelWaitingForFundingTx =
    ...
    member this.CreateFundingTx
        (fundingTx: FinalizedTx)
        (outIndex: TxOutIndex)
        : Result<FundingCreatedMsg * ChannelWaitingForFundingSigned, ChannelError> =
        result {
            let localParams = this.LocalParams
            let remoteParams = this.RemoteParams

            let commitmentSpec =
                CommitmentSpec.Create
            ...
            let channelId =
                OutPoint(fundingTxId.Value, uint32 outIndex.Value)
                    .ToChannelId()
...
and ChannelWaitingForAcceptChannel =
    ...
    member this.ApplyAcceptChannel
        (msg: AcceptChannelMsg)
        : Result<IDestination * Money * ChannelWaitingForFundingTx, ChannelError> =
        result {
            do!
            ...
            let remoteParams =
                RemoteParams.FromAcceptChannel this.RemoteInit msg
...
and Channel =
    ...
    member internal this.RemoteNextCommitInfoIfFundingLockedNormal
        (operation: string)
        : Result<RemoteNextCommitInfo, ChannelError> =
        ...
        | None ->
            sprintf
                ...
                operation
            |> apiMisuse
        | Some _ -> this.RemoteNextCommitInfoIfFundingLocked operation

    static member NewOutbound
        (
            channelHandshakeLimits: ChannelHandshakeLimits,
            channelOptions: ChannelOptions,
            announceChannel: bool,
        ...
        let openChannelMsgToSend: OpenChannelMsg =
            {
                ...
                RevocationBasepoint =
                    channelPrivKeys.RevocationBasepointSecret.RevocationBasepoint
                        ()
                PaymentBasepoint =
                    channelPrivKeys.PaymentBasepointSecret.PaymentBasepoint()
                DelayedPaymentBasepoint =
                    channelPrivKeys.DelayedPaymentBasepointSecret.DelayedPaymentBasepoint
                        ()
                HTLCBasepoint =
                    channelPrivKeys.HtlcBasepointSecret.HtlcBasepoint()
                FirstPerCommitmentPoint =
        ...
        result {
            do!
                Validation.checkOurOpenChannelMsgAcceptable
                    localParams
    ...
    static member NewInbound
        ...
        result {
            ...
            let acceptChannelMsg: AcceptChannelMsg =
                {
                    ...
                    MaxAcceptedHTLCs = localParams.MaxAcceptedHTLCs
                    FundingPubKey =
                        channelPrivKeys.FundingPrivKey.FundingPubKey()
                    RevocationBasepoint =
                        channelPrivKeys.RevocationBasepointSecret.RevocationBasepoint
                            ()
                    PaymentBasepoint =
            ...
            let remoteChannelPubKeys =
                {
                    ...
                    DelayedPaymentBasepoint =
                        openChannelMsg.DelayedPaymentBasepoint
                    HtlcBasepoint = openChannelMsg.HTLCBasepoint
                }

            let remoteParams =
                RemoteParams.FromOpenChannel remoteInit openChannelMsg
            ...
            let channelWaitingForFundingCreated =
                {
                    AnnounceChannel =
                        openChannelMsg.ChannelFlags.AnnounceChannel
                    RemoteNodeId = remoteNodeId
                    Network = network
                    FundingTxMinimumDepth = minimumDepth
                    LocalStaticShutdownScriptPubKey = shutdownScriptPubKey
                    RemoteStaticShutdownScriptPubKey =
                        openChannelMsg.ShutdownScriptPubKey()
                    IsFunder = false
                    ChannelOptions = channelOptions
                    ChannelPrivKeys = channelPrivKeys
                    NodeSecret = nodeSecret
                    ...
                    RemoteFirstPerCommitmentPoint =
                        openChannelMsg.FirstPerCommitmentPoint
                }
    ...
    member this.ApplyFundingLocked
                ...
                { this with
                    RemoteNextCommitInfo =
                        RemoteNextCommitInfo.Revoked
                            fundingLockedMsg.NextPerCommitmentPoint
                        |> Some
                }
        }

    member this.ApplyFundingConfirmedOnBC
        (height: BlockHeight)
        (txindex: TxIndexInBlock)
        (depth: BlockHeightOffset32)
        : Result<Channel * FundingLockedMsg, ChannelError> =
        result {
            let requiredDepth =
                this.SavedChannelState.StaticChannelConfig.FundingTxMinimumDepth
    ...
    member this.AddHTLC
        (op: OperationAddHTLC)
        : Result<Channel * UpdateAddHTLCMsg, ChannelError> =
        result {
            if this.NegotiatingState.HasEnteredShutdown() then
                return!
                    sprintf
                        "Could not add new HTLC %A since shutdown is already in progress."
                        op
                    |> apiMisuse
            else
                do!
                    Validation.checkOperationAddHTLC
                        this.SavedChannelState.StaticChannelConfig.RemoteParams
                        op
    ...
    member this.ApplyUpdateAddHTLC
    ...
    member this.FulFillHTLC
        (cmd: OperationFulfillHTLC)
        : Result<Channel * UpdateFulfillHTLCMsg, ChannelError> =
        result {
            let! remoteNextCommitInfo =
    ...
    member this.ApplyUpdateFulfillHTLC
        (msg: UpdateFulfillHTLCMsg)
        : Result<Channel, ChannelError> =
        result {
            let! remoteNextCommitInfo =
                this.RemoteNextCommitInfoIfFundingLockedNormal
                    "ApplyUpdateFulfullHTLC"
    ...
    member this.FailHTLC
        (op: OperationFailHTLC)
        : Result<Channel * UpdateFailHTLCMsg, ChannelError> =
        result {
            let! remoteNextCommitInfo =
    ...
    member this.FailMalformedHTLC
        (op: OperationFailMalformedHTLC)
        : Result<Channel * UpdateFailMalformedHTLCMsg, ChannelError> =
        result {
            let! remoteNextCommitInfo =
                this.RemoteNextCommitInfoIfFundingLockedNormal
                    "FailMalformedHTLC"
    ...
    member this.ApplyUpdateFailHTLC
    ...
    member this.UpdateFee
        (op: OperationUpdateFee)
        : Result<Channel * UpdateFeeMsg, ChannelError> =
        result {
    ...
    member this.ApplyUpdateFee
        ...
        result {
            ...
            let localFeerate =
                this.ChannelOptions.FeeEstimator.GetEstSatPer1000Weight(
    ...
    member this.SignCommitment
        ()
        : Result<Channel * CommitmentSignedMsg, ChannelError> =
        result {
            let! remoteNextCommitInfo =
    ...
    member this.ApplyCommitmentSigned
        (msg: CommitmentSignedMsg)
        : Result<Channel * RevokeAndACKMsg, ChannelError> =
        result {
            let! _remoteNextCommitInfo =
                this.RemoteNextCommitInfoIfFundingLockedNormal
    ...
    member this.ApplyRevokeAndACK
        ...
        result {
            ...
            match remoteNextCommitInfo with
            | RemoteNextCommitInfo.Waiting _ when
                (msg.PerCommitmentSecret.PerCommitmentPoint()
                 <> this.SavedChannelState.RemoteCommit.RemotePerCommitmentPoint)
                ->
                let errorMsg =
                    sprintf
                        "Invalid revoke_and_ack %A; must be %A"
            ...
            | RemoteNextCommitInfo.Waiting waitingForRevocation ->
                ...
                | Error err ->
                    return! Error <| invalidRevokeAndACK msg err.Message
                        ...
                        { this.SavedChannelState with
                            ...
                            LocalChanges =
                                { this.SavedChannelState.LocalChanges with
                                    ...
                                    ACKed =
                                        this.SavedChannelState.LocalChanges.ACKed
                                        @ this.SavedChannelState.LocalChanges.Signed
                                }
                            RemoteChanges =
    ...
    member this.Close
        ...
        : Result<Channel * ShutdownMsg, ChannelError> =
        result {
            ...
            if (this.Commitments.LocalHasUnsignedOutgoingHTLCs()) then
                do!
                    Error
                    <| cannotCloseChannel
                        "Cannot close with unsigned outgoing htlcs"
    ...
    member internal this.MakeClosingTx
        ...
        result {
            ...
            let localSignature, psbtUpdated =
                channelPrivKeys.SignWithFundingPrivKey closingTx.Value
    ...
    member internal this.FirstClosingFee
        (localSpk: ShutdownScriptPubKey)
        (remoteSpk: ShutdownScriptPubKey)
        =
        result {
            ...
            tx.Inputs.[0].WitScript <- let witness =
                                           seq
                                               [
                                                   Channel.DummySig.ToBytes()
                                                   Channel.DummySig.ToBytes()
                                                   dummyClosingTx.Value.Inputs.[0]
    ...
    static member internal NextClosingFee
        (
            localClosingFee: Money,
            remoteClosingFee: Money
        ) =
    ...
    member this.RemoteShutdown
        (msg: ShutdownMsg)
        (localShutdownScriptPubKey: ShutdownScriptPubKey)
        : Result<Channel * Option<ShutdownMsg> * Option<ClosingSignedMsg>, ChannelError> =
        result {
            ...
            // Do we have Unsigned Outgoing HTLCs?
                ...
                let hasNoPendingHTLCs =
                    ...
                    | None -> true
                    | Some remoteNextCommitInfo ->
                        this.SavedChannelState.HasNoPendingHTLCs
                            remoteNextCommitInfo
                ...
                if hasNoPendingHTLCs then
                    ...
                    if this.SavedChannelState.StaticChannelConfig.IsFunder then
                        ...
                        return
                            channel, localShutdownMsgOpt, Some closingSignedMsg
                    else
                        let nextState =
                            {
                                LocalRequestedShutdown =
    ...
    member this.ApplyClosingSigned
        ...
        result {
            ...
            let maybeLocalFee =
                this.NegotiatingState.LocalClosingFeesProposed |> List.tryHead

            let areWeInDeal = Some(msg.FeeSatoshis) = maybeLocalFee

            let hasTooManyNegotiationDone =
                (this.NegotiatingState.LocalClosingFeesProposed |> List.length)
                >= this.ChannelOptions.MaxClosingNegotiationIterations

            if (areWeInDeal || hasTooManyNegotiationDone) then
                return this, MutualClose(finalizedTx, None)
            else
                let lastLocalClosingFee =
                    ...
                    let nextState =
                        { this.NegotiatingState with
                            ...
                            RemoteClosingFeeProposed =
                                Some(msg.FeeSatoshis, msg.Signature)
                        }
    ...
    member private this.sendCommit
        ...
        | RemoteNextCommitInfo.Revoked remoteNextPerCommitmentPoint ->
            result {
                ...
                let signature, _ =
                    channelPrivKeys.SignWithFundingPrivKey remoteCommitTx.Value

                let sortedHTLCTXs =
                    Commitments.Helpers.sortBothHTLCs
                        htlcTimeoutTxs
                        htlcSuccessTxs
                ...
                let nextRemoteCommitInfo =
                    { savedChannelState.RemoteCommit with
                        Index =
                            savedChannelState.RemoteCommit.Index.NextCommitment
                                ()
        ...
        | RemoteNextCommitInfo.Waiting _ -> CanNotSignBeforeRevocation |> Error

    member private this.receiveCommit
        (msg: CommitmentSignedMsg)
        ...
        if this.RemoteHasChanges() |> not then
            ...
            let remoteChannelKeys =
                savedChannelState.StaticChannelConfig.RemoteChannelPubKeys
            ...
            result {
                ...
                let sigPair =
                    let localSigPair =
                        seq
                            [
                                (localChannelKeys.FundingPubKey.RawPubKey(),
                                 signature)
                            ]

                    let remoteSigPair =
                        seq[(remoteChannelKeys.FundingPubKey.RawPubKey(),
                             TransactionSignature(
                                 msg.Signature.Value,
                                 SigHash.All
                             ))]
                ...
                let remoteHTLCPubKey =
                    localPerCommitmentPoint.DeriveHtlcPubKey
                        remoteChannelKeys.HtlcBasepoint
                ...
                let nextCommitments =
                    let completedOutgoingHTLCs =
                        let t1 =
                            savedChannelState.LocalCommit.Spec.OutgoingHTLCs
                            |> Map.toSeq
                            |> Seq.map(fun (k, _) -> k)
                            |> Set.ofSeq

                        let t2 =
                            localCommit1.Spec.OutgoingHTLCs
                            |> Map.toSeq
                            |> Seq.map(fun (k, _) -> k)
                            |> Set.ofSeq
