...
module internal Feature =
    ...
    let hasFeature
        (features: BitArray)
        (f: Feature)
        (support: option<FeaturesSupport>)
        =
        match support with
        | Some(Mandatory) -> isFeatureOn (features) (f.Mandatory)
        | Some(Optional) -> isFeatureOn (features) (f.OptionalBitPosition)
        | None ->
    ...
    let validateFeatureGraph(features: BitArray) =
        result {
            for kvp in featuresDependency do
                ...
                if hasFeature (features) (f) (None)
                   && deps
                      |> List.exists(fun d ->
                          not <| hasFeature (features) (d) (None)
                      ) then
                    return!
                        sprintf
...
type FeatureBits private (bitArray: BitArray) =
    member __.BitArray
        with internal get () = bitArray

    member this.ByteArray
        with internal get () = this.BitArray.ToByteArray()
    ...
    static member TryCreate(ba: BitArray) =
        result {
            ...
            if not <| Feature.areSupported(ba) then
                return!
                    sprintf
                        "feature bits (%s) contains a mandatory flag that we don't know!"
                        (ba.PrintBits())
    ...
    static member TryCreate(bytes: array<byte>) =
    ...
    static member private Unwrap(r: Result<FeatureBits, _>) =
        ...
        | Error(FeatureError.BogusFeatureDependency e) ->
    ...
    static member CreateUnsafe(bytes: array<byte>) =
    ...
    member this.PrettyPrint =
        ...
        for f in Feature.allFeatures do
            if (reversed.Length > f.MandatoryBitPosition)
               && (reversed.[f.MandatoryBitPosition]) then
                sb.Append(sprintf "%s is mandatory. " f.RfcName) |> ignore
            else if (reversed.Length > f.OptionalBitPosition)
                    && (reversed.[f.OptionalBitPosition]) then
    ...
    member this.CompareTo(o: FeatureBits) =
        if (this.BitArray.Length > o.BitArray.Length) then
            -1
        else if (this.BitArray.Length < o.BitArray.Length) then
            1
        else
    ...
    interface IComparable with
        member this.CompareTo o =
            ...
            | _ ->
                raise
                <| ArgumentException(
                    "Comparison should be done against same type (FeatureBits)",
                    "o"
                )
// --------
